...
class DisconnectChecker:
    ...
    def initialize_user(self, uid: int):
        self.printer.print("DisconnectChecker:Initialize", "Now initializing user", str(uid))
    ...
    def create_pairing(self, uid: int):
        function_name = "DisconnectChecker:CreatePairing"
        try:
            ...
            if curr_user.get_partner() != -1:
                ...
                self.printer.print(function_name, "User has a partner already, user {}".format(curr_user.get_partner()), uid=uid)
                ...
                self.printer.print(function_name, "User timed out.", uid)
            ...
            for user_key in self.users:
                ...
                if user is not curr_user and user.get_partner() == -1:
                    ...
                    self.printer.print(function_name, "Users {} and {} were paired!".format(str(curr_user.get_id()), str(user.get_id())), uid)
                    self.user_table_lock.release()
            ...
            self.printer.print("checker:create_pairing", repr(error))
            raise
    ...
    def conv_print(self, uid: int, printer):
        if uid in self.users:
            ...
            self.printer.print("conv_print", "User not found", uid=uid)

    # Remove the user from the "current users" list once they complete the task
    def safe_delete_user(self, uid: int, success: bool):
        try:
            if success:
                self.printer.print("Safe Delete User", "User " + str(uid) + " has completed the task! Congratulations!", uid)
            ...
            self.printer.print("Checker:safe_delete_user", repr(error))
            raise
    ...
    def unsafe_delete_user(self, uid: int):
        try:
            self.printer.print("Unsafe Delete User", "User " + str(uid) + " has timed out.", uid)
            curr_user = self.users[uid]
            pid = curr_user.get_partner()
            if (pid != -1 and pid != -2):
                partner = self.users[pid]
            ...
            self.printer.print("Checker:unsafe_delete_user", repr(error))
            raise
    ...
    def ping_user(self, uid: int):
        ...
        try:
            ...
            self.printer.print("Checker:ping_user", repr(error))
            raise
    ...
    def check_for_disconnects(self):
        try:
            ...
            self.printer.print("Checker:check_for_disconnects", repr(error))
            raise

    # Remove a disconnected user from the user table.
    def remove_user(self, uid: int):
        try:
            self.printer.print("Remove User", "Removing a user from the users table", uid=uid)
            ...
            self.printer.print("Checker:remove_user", repr(error))
            raise
